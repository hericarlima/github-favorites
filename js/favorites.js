...
export class FavoritesView extends Favorites {
    ...
    update() {
        ...
        if(this.entries.length <= 0) {
            this.tbody?.append(this.createEmpty())
        } else {
            this.entries.forEach( user => {
                const row = this.createRow()

                row.querySelector('.user img').src = `http://GitHub.com/${user.login}.png`
                row.querySelector('.user img').alt = `foto de usuário de ${user.name}`
                ...
                row.querySelector('.remove').onclick = () => {
                    const removeOk = confirm('Tem certeza que deseja deletar este favorito?')
    ...
    createRow() {
        ...
        const contentRow = `
        <td class="user">
            <img src="http://github.com/hericarlima.png" alt="foto de usuário de Hérica Lima">
            <a href="http://github.com/hericarlima" target="_black">
            <p>Hérica Lima</p>
            <span>/hericarlima</span>
            </a>
        </td>
